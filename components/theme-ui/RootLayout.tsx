...
import Notifications from '@/components/url-components/Notifications'
      ...
      <Container>
        <div className="flex items-center justify-between">
          ...
          <div className="flex items-center gap-x-8">
            ...
            <Link href="/">Social Media Previewer</Link>
          ...
          <div className="flex items-center gap-x-8">
            <div className="flex gap-7">
              ...
              {/* <Link href="/pricing" aria-label="Home">
                Pricing
              </Link> */}
            ...
            >
              Images Kit <span className="text-[0.5rem] ml-1">SOON</span>
            </Link> */}
              {/* <Link
                href="/"
                aria-label="Home"
              ...
              >
                DNS Kit <span className="ml-1 text-[0.5rem]">SOON</span>
              </Link> */}
            </div>
            <div className="z-50">
              <Button className="sm:hidden md:hidden lg:block" href="/sign-up" invert={invert}>
                Sign Up
              </Button>
            </div>
...
function Navigation() {
  return (
    <nav className="mt-px font-display text-5xl font-medium tracking-tight text-white">
      {/* <NavigationRow>
        <NavigationItem href="/">Social Media Previewer</NavigationItem>
        <NavigationItem href="/">Social Media Previewer</NavigationItem>
      </NavigationRow> */}
    </nav>
  )
...
function RootLayoutInner({ children, pathname }: { children: React.ReactNode; pathname: string }) {
  ...
  const isPreviewerPath = pathname.startsWith('/previewer')
  ...
  return (
    <div className={cn('', isPreviewerPath && 'bg-[#f5f5f7] bg-opacity-50')}>
      <MotionConfig transition={shouldReduceMotion ? { duration: 0 } : undefined}>
        <header>
          <div
            className={cn('absolute left-0 right-0 top-2 z-10 pt-14', isPreviewerPath && 'bg-[#f5f5f7] bg-opacity-50')}
            aria-hidden={expanded ? 'true' : undefined}
            // @ts-ignore (https://github.com/facebook/react/issues/17157)
            inert={expanded ? '' : undefined}
          >
            <Header
              isPreviewerPath={isPreviewerPath}
              panelId={panelId}
              icon={MenuIcon}
              toggleRef={openRef}
              expanded={expanded}
                  ...
                  <div className="grid grid-cols-1 gap-y-10 pb-16 pt-10 sm:grid-cols-2 sm:pt-16">
                    <div>
                      ...
                      <h2 className="font-display text-base font-semibold text-white">
                        Suggest a feature or Leave feedback
                      </h2>
                      <Offices invert className="mt-6 grid grid-cols-1 gap-8 sm:grid-cols-2" />
                    </div>
                    <div className="sm:border-l sm:border-transparent sm:pl-16">
                      <h2 className="font-display text-base font-semibold text-white">Bookmark this website</h2>
        ...
        >
          <motion.div layout className="relative isolate flex w-full flex-col pt-9">
            ...
            <main className={cn('w-full flex-auto', isPreviewerPath && 'bg-[#f5f5f7] bg-opacity-50')}>{children}</main>
            <Footer isPreviewerPath={isPreviewerPath} />
          </motion.div>
        </motion.div>
      </MotionConfig>
    </div>
